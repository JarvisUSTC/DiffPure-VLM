...
# 限制可见GPU
nohup bash -c "CUDA_VISIBLE_DEVICES=4 python qwen25vl_general_inference.py --model_path "Qwen/Qwen2.5-VL-7B-Instruct" --gpu-id 0 --image_file $image_prompt_path --input_file harmful_corpus/rtp_prompts.jsonl --output_file $output_path/rtp_prompts_challenges.jsonl" > $output_path/output.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=4 python get_metric.py --input $output_path/rtp_prompts_challenges.jsonl --output $output_path/rtp_prompts_challenges_metrics.jsonl

echo "Evaluated the generated prompts"

# cal metric
CUDA_VISIBLE_DEVICES=4 python cal_metrics_markdown.py --input $output_path/rtp_prompts_challenges_metrics.jsonl
