...
do
  j=$((i-9))
  # 限制可见GPU
  nohup bash -c "CUDA_VISIBLE_DEVICES=$i python minigpt_inference_diffpure.py --cfg-path eval_configs/minigpt4_eval.yaml --gpu-id 0 --image_file $image_prompt_path --input_file harmful_corpus/rtp_prompts_challenges_$j.jsonl --output_file $output_path/rtp_prompts_challenges_$j.jsonl --def_max_timesteps 30 --def_num_denoising_steps 30 --def_sampling_method ddpm" > $j.log 2>&1 &
done
