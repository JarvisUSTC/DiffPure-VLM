...
# do
#   # 限制可见GPU
#   nohup bash -c "CUDA_VISIBLE_DEVICES=$i python minigpt_inference.py --cfg-path eval_configs/minigpt4_eval.yaml --gpu-id 0 --image_file $image_prompt_path --input_file harmful_corpus/rtp_prompts_challenges_$i.jsonl --output_file $output_path/rtp_prompts_challenges_$i.jsonl" > $i.log 2>&1 &
# done
...
nohup bash -c "CUDA_VISIBLE_DEVICES=0 python minigpt_inference.py --cfg-path eval_configs/minigpt4_eval.yaml  --gpu-id 0 --image_file $image_prompt_path --input_file harmful_corpus/rtp_prompts.jsonl --output_file $output_path/rtp_prompts_challenges.jsonl" > $output_path/output.log 2>&1 &
