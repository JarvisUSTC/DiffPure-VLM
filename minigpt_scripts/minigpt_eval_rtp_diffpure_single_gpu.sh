...
nohup bash -c "CUDA_VISIBLE_DEVICES=0 python minigpt_inference_diffpure.py --cfg-path eval_configs/minigpt4_eval.yaml  --gpu-id 0 --image_file $image_prompt_path --input_file harmful_corpus/rtp_prompts.jsonl --output_file $output_path/rtp_prompts_challenges.jsonl --def_max_timesteps $timesteps --def_num_denoising_steps $timesteps --def_sampling_method ddpm" > $output_path/output.log 2>&1 &
